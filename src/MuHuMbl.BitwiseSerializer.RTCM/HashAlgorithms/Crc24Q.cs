namespace MuHuMbl.BitwiseSerializer.RTCM.HashAlgorithms
{
    public class Crc24Q : IHashAlgorithm
    {
        ...
        private readonly uint _crcSeed = 0;
        private readonly uint _crcPoly = 0x1864CFBu;
        ...
        private static readonly uint[] _crc24q =
        {
        ...
        public uint GetHash(byte[] data)
        {
            ...
            {
                crc = ((crc << 8) & 0x00ffffff) ^ _crc24q[data[i] ^ (byte)(crc >> 16)];
            }
        ...
        {
            var crc = GetHash(data.Take(data.Length -3).ToArray());

            return data[^3] == Hi(crc) && data[^2] == Mid(crc) && data[^1] == Lo(crc);
        }

        private static byte Lo(uint x) => (byte) (x & 0xff);
        private static byte Mid(uint x) => (byte)((x >> 8) & 0xff);
        private static byte Hi(uint x) => (byte)((x >> 16) & 0xff);
    }
}